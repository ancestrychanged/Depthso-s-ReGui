...
ReGui.Icons = {
	Dot = "rbxassetid://15264357731",
	Arrow = "rbxassetid://11422144208",
	Close = "rbxassetid://11452902996",
	Checkmark = "rbxassetid://15264344933",
...
function ReGui:LoadPrefabs(): Folder?
	...
	--// Check script for prefabs (wrapped in pcall for standalone environments)
	local Success, ScriptUi = pcall(function()
		return script:WaitForChild(Name, 0.1)
	end)
	if Success and ScriptUi then return ScriptUi end
	...
	--// Fallback: Dynamic asset loading
	local PrefabsId = self.PrefabsId
	if PrefabsId then
		self:Warn("Attempting to load prefabs from asset ID:", PrefabsId)
		local Success, Result = pcall(function()
			return game:GetObjects(`rbxassetid://{PrefabsId}`)
		end)
		
		if Success and Result and Result[1] then
			self:Warn("Prefabs loaded successfully from asset!")
			local Folder = Result[1]
			Folder.Name = Name
			return Folder
		else
			self:Warn("GetObjects failed. Success:", Success, "Result:", Result)
		end
	end

	self:Warn("CRITICAL: No prefabs could be loaded. ReGui UI elements will not function.")
	return nil
end
...
function ReGui:InsertPrefab(Name: string, Properties): GuiObject
	...
	if not Folder then
		self:Warn(`Failed to insert prefab ({Name}): Main prefabs folder is missing`)
		return nil
	end

	local Prefabs = Folder:FindFirstChild("Prefabs")
	if not Prefabs then 
		self:Warn(`Failed to insert prefab ({Name}): Internal 'Prefabs' folder is missing from {Folder}`)
		return nil 
	end

	local Prefab = Prefabs:FindFirstChild(Name)
	if not Prefab then
		self:Warn(`Failed to insert prefab ({Name}): Prefab not found in {Prefabs}`)
		return nil
	end
...
function ReGui:WrapGeneration(Function, Data: table)
	...
	return function(Canvas, Flags, ...)
		...
		if Success == false then
			...
			--// Recursion protection: Don't attempt to create an error element if we are already handling an error
			if Canvas.Error and not ReGui._InError then
				ReGui._InError = true
				pcall(function()
					Canvas:Error({
						Parent = Parent,
...
--// CodeEditor is an alias for Console with additional methods for code editing
export type CodeEditor = Console & {
	GetText: (CodeEditor) -> string,
	SetText: (CodeEditor, Value: string) -> CodeEditor,
}
ReGui:DefineElement("CodeEditor", {
	...
	Create = function(self, Config: CodeEditor): CodeEditor
		--// Create using Console as base
		local Class, Object = self:Console(Config)
		
		--// Add GetText alias for compatibility
		function Config:GetText()
			return self:GetValue()
		end
		
		--// Add SetText alias for compatibility
		function Config:SetText(Value: string)
